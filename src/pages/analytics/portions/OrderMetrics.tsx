...
const OrderMetrics: React.FC = () => {
  const {data:userAnalytics ,isLoading,error} = useQuery({
    ...
    queryFn: () => fetchAnalytics('OrderAnalytics'),
  });
  ...
  const pieData = {
    labels: ['Total Orders', 'Total Delivered', 'Total Cancelled'],
    datasets: [
      {
  ...
  return (
    <div className='flex flex-col gap-6 p-6'>
      ...
      <BoxContainer data={userAnalytics.cardData} heading='Order Metrics' />
    </div>
  )
}
